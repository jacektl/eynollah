...
def return_x_start_end_mothers_children_and_type_of_reading_order(x_min_hor_some,x_max_hor_some,cy_hor_some,peak_points,cy_hor_diff):
    ...
    if len(new_main_sep_y)>0:
        ...
        y_mains_sep_no_borders=[]

        for ii in range(len(new_main_sep_y)):
            y_mains.append(y_sep[new_main_sep_y[ii]])
            y_mains_sep_no_borders.append(y_sep[new_main_sep_y[ii]])
        ...
        if (y_min_new in y_mains_sep_no_borders) and (y_max_new in y_mains_sep_no_borders):
        ...
        elif (y_min_new in y_mains_sep_no_borders) and (y_max_new not in y_mains_sep_no_borders):
            #print('burda')
            x_start=x_start[(y_sep>y_min_new) & (y_sep<=y_max_new)]
            #print('burda1')
            x_end=x_end[(y_sep>y_min_new) & (y_sep<=y_max_new)]
            ...
            kind=kind[(y_sep>y_min_new) & (y_sep<=y_max_new)]
            y_sep=y_sep[(y_sep>y_min_new) & (y_sep<=y_max_new)]
        elif (y_min_new not in y_mains_sep_no_borders) and (y_max_new in y_mains_sep_no_borders):
            x_start=x_start[(y_sep>=y_min_new) & (y_sep<y_max_new)]
            x_end=x_end[(y_sep>=y_min_new) & (y_sep<y_max_new)]
            kind=kind[(y_sep>=y_min_new) & (y_sep<y_max_new)]
            y_sep=y_sep[(y_sep>=y_min_new) & (y_sep<y_max_new)]
    ...
    return reading_order_type,x_start_returned, x_end_returned ,y_sep_returned,y_diff_returned,y_lines_without_mother,x_start_without_mother,x_end_without_mother,there_is_sep_with_child,y_lines_with_child_without_mother,x_start_with_child_without_mother,x_end_with_child_without_mother
...
def boosting_headers_by_longshot_region_segmentation(textregion_pre_p, textregion_pre_np, img_only_text):
    ...
    # textregion_pre_p[:,:,0][( img_only_text[:,:]==1) & (textregion_pre_p[:,:,0]!=7)  & (textregion_pre_p[:,:,0]!=2)]=1 # earlier it was so, but by this manner the drop capitals are false deleted
...
def return_hor_splitter_by_index_for_without_verticals(peaks_neg_fin_t, x_min_hor_some, x_max_hor_some):
        ...
        if len(min_h_neg) > 0 and len(max_h_neg) > 0:
            ...
            if len(deletions) > 0:
                ...
                for j in range(len(deletions)):
                    ...
                    unique_deletes = np.unique(indexes_to_delete)
                    # print(min_h_neg[0],unique_deletes)
                    unique_deletes_int = unique_deletes[unique_deletes < min_h_neg[0]]

                indexer_lines_deletions_len.append(len(deletions))
                indexer_uniq_ind.append([deletions])

            else:
                indexer_lines_deletions_len.append(0)
                indexer_uniq_ind.append(-999)

            index_line_true = min_h_neg[0] - len(unique_deletes_int)
            ...
            if len(unique_deletes_int) > 0:
                for dd in range(len(unique_deletes_int)):
                    indexes_to_delete.append(unique_deletes_int[dd])
        else:
    ...
    return indexer_lines, peaks_true, arg_min_hor_sort, indexer_lines_deletions_len, indexer_uniq_ind
...
def find_num_col(regions_without_separators, multiplier=3.8):
    ...
    if num_col == 3:
        if (peaks_neg_true[0] > p_g_u and peaks_neg_true[1] > p_g_u) or (peaks_neg_true[0] < p_g_l and peaks_neg_true[1] < p_g_l) or (peaks_neg_true[0] < p_m and (peaks_neg_true[1] + 200) < p_m) or ((peaks_neg_true[0] - 200) > p_m and peaks_neg_true[1] > p_m):
            num_col = 1
            peaks_neg_true = []
        elif (p_g_u > peaks_neg_true[0] > p_g_l) and (peaks_neg_true[1] > p_u_quarter):
            peaks_neg_true = [peaks_neg_true[0]]
        elif (p_g_u > peaks_neg_true[1] > p_g_l) and (peaks_neg_true[0] < p_quarter):
...
def delete_separator_around(splitter_y, peaks_neg, image_by_region):
    ...
    if len(image_by_region.shape) == 3:
        for i in range(len(splitter_y) - 1):
            for j in range(1, len(peaks_neg[i]) - 1):
                image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 0][image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 0] == 6] = 0
                image_by_region[splitter_y[i] : splitter_y[i + 1], peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 0][image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 1] == 6] = 0
                image_by_region[splitter_y[i] : splitter_y[i + 1], peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 0][image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 2] == 6] = 0

                image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 0][image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 0] == 7] = 0
                image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 0][image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 1] == 7] = 0
                image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 0][image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j]), 2] == 7] = 0
    else:
        for i in range(len(splitter_y) - 1):
            for j in range(1, len(peaks_neg[i]) - 1):
                image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j])][image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j])] == 6] = 0

                image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j])][image_by_region[int(splitter_y[i]) : int(splitter_y[i + 1]), peaks_neg[i][j] - int(1.0 / 20.0 * peaks_neg[i][j]) : peaks_neg[i][j] + int(1.0 / 20.0 * peaks_neg[i][j])] == 7] = 0
...
def return_hor_splitter_by_index(peaks_neg_fin_t, x_min_hor_some, x_max_hor_some):
...
def find_number_of_columns_in_document(region_pre_p, num_col_classifier, pixel_lines, contours_h=None):
    ...
    for iteils in args_big_parts:
        ...
        regions_without_separators_teil=regions_without_separators[int(splitter_y_new[iteils]):int(splitter_y_new[iteils+1]),:,0]
        #image_page_background_zero_teil=image_page_background_zero[int(splitter_y_new[iteils]):int(splitter_y_new[iteils+1]),:]

        #print(regions_without_separators_teil.shape)
        ##plt.imshow(regions_without_separators_teil)
        ##plt.show()

        #num_col, peaks_neg_fin=self.find_num_col(regions_without_separators_teil,multiplier=6.0)

        #regions_without_separators_teil=cv2.erode(regions_without_separators_teil,kernel,iterations = 3)
        #
        num_col, peaks_neg_fin=find_num_col(regions_without_separators_teil,multiplier=7.0)
        ...
        if 1>0:#len( matrix_new[:,9][matrix_new[:,9]==1] )>0 and np.max(matrix_new[:,8][matrix_new[:,9]==1])>=0.1*(np.abs(splitter_y_new[i+1]-splitter_y_new[i] )):
            ...
            if (reading_order_type==1) or (reading_order_type==0 and (len(y_lines_without_mother)>=2 or there_is_sep_with_child==1)):
                ...
                try:
                    ...
                    #check if there is a big separater in this y_mains_sep_no_borders
                    ...
                    for il in range(len(y_lines_by_order)):
                        ...
                        for column in range(x_start_itself,x_end_itself+1):
                            ...
                            for yic in range(len(y_copy)):
                                #print('burda')
                                if y_copy[yic]>y_itself and x_start_copy[yic] <= column <= x_end_copy[yic]:
