...
def filter_small_drop_capitals_from_no_patch_layout(layout_no_patch, layout1):
    ...
    for jj in range(len(contours_drop_parent)):
        ...
        iou_of_box_and_contour = float(drop_only.shape[0] * drop_only.shape[1]) * areas_cnt_text[jj] / float(w * h) * 100
        height_to_weight_ratio = h / float(w)
        weigh_to_height_ratio = w / float(h)

        if iou_of_box_and_contour > 60 and weigh_to_height_ratio < 1.2 and height_to_weight_ratio < 2:
